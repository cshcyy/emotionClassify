...
def getData():
	...
	df = pd.DataFrame(ecg_signals)  # 将所有信号构成的特征向量列表变成dataframe表格形式


	return df,ecg_labels
# 载入数据
...
def baseline_model():
	model = Sequential()
	model.add(Conv1D(16, 3, input_shape=(150, 1)))
...
with open(r"D:\pycharm\model.json", 'w')as json_file:
	json_file.write(model_json)  # 权重不在json中,只保存网络结构
estimator.model.save_weights('model.h5')

# 加载模型用做预测
json_file = open(r"D:\pycharm\model.json", "r")
