...
def selectRFParam2():
	clf_GBDT = GradientBoostingClassifier(n_estimators=270 )
...
def selectRFParam3():
	clf_GBDT = GradientBoostingClassifier(n_estimators=270,max_depth=2,min_samples_split=86 )
	param_grid = { 'min_samples_leaf':range(20,101,10)}
	gsearch1 = GridSearchCV(clf_GBDT, param_grid=param_grid, n_jobs=4,cv=5)
	T = getData_2()  # 获取数据集
	gsearch1.fit(T[0], T[1])  # 传入训练集矩阵和训练样本类标
	means = gsearch1.cv_results_['mean_test_score']
	...
	print("Best score on train set:{:.2f}".format(gsearch1.best_score_))
def selectRFParam4():
	clf_GBDT = GradientBoostingClassifier(n_estimators=270,max_depth=2,min_samples_split=86 ,min_samples_leaf=30)
...
def selectRFParam5():
	clf_GBDT = GradientBoostingClassifier(n_estimators=270,max_depth=2,min_samples_split=86 ,min_samples_leaf=30,max_features=8)
